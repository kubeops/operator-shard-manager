...
PRODUCT_NAME       := operator-shard-manager
ENFORCE_LICENSE    ?=

GO_PKG   := kubeops.dev
REPO     := $(notdir $(shell pwd))
BIN      := operator-shard-manager
COMPRESS ?= no
...
uninstall:
	@cd ../installer; \
	helm uninstall operator-shard-manager --namespace=$(KUBE_NAMESPACE) || true
...
docker-certify-redhat:
	@preflight check container $(IMAGE):$(VERSION_UBI) \
		--submit \
		--certification-component-id=6957a66e32888f9fd4143671
